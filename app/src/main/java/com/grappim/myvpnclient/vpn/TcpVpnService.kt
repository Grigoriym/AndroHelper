...
class TcpVpnService(
  ...
  private val parentJob: CompletableJob = Job()
  ...
  fun start() {
    coroutineScope.launch {
      whileSelect {
        outputCh.onReceive { value ->
          coroutineScope.launch { serveOutput(value) }
          true
        }
    ...
    coroutineScope.launch { readLoop() }
    ...
    coroutineScope.async {
      ...
      when {
        ...
        tcpPacket.header.ack -> {
          ...
          Log.d(
            TAG,
            "request payload: ${tcpPacket.payload.rawData.size} bytes\n${tcpPacket.payload.rawData}\n${String(
              tcpPacket.payload.rawData
            )}"
          )
          if (!conn!!.waitingForNetworkData) {
          ...
          conn!!.tcpHeader.seqNum = tcpPacket.header.acknowledgmentNumber
          conn!!.tcpHeader.ackNum =
            tcpPacket.header.sequenceNumber.plus(tcpPacket.payload.rawData.size)
